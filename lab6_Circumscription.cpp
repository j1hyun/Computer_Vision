...
{
	for (int i = LU_X; i < RD_X; i++) 	Img[LU_Y * W + i] = 140;
	for (int i = LU_X; i < RD_X; i++) 	Img[RD_Y * W + i] = 140;
	for (int i = LU_Y; i < RD_Y; i++) 	Img[i * W + LU_X] = 140;
	for (int i = LU_Y; i < RD_Y; i++) 	Img[i * W + RD_X] = 140;
}

void DrawRedRectOutline(BYTE* Img, int W, int H, int LUX, int LUY, int RDX, int RDY)
{
	for (int i = LUX; i < RDX; i++) 	Img[LUY * W * 3 + i * 3] = 0;
	for (int i = LUX; i < RDX; i++) 	Img[LUY * W * 3 + i * 3 + 1] = 0;
	for (int i = LUX; i < RDX; i++)  Img[LUY * W * 3 + i * 3 + 2] = 255;
	for (int i = LUX; i < RDX; i++) 	Img[RDY * W * 3 + i * 3] = 0;
	for (int i = LUX; i < RDX; i++)  Img[RDY * W * 3 + i * 3 + 1] = 0;
	for (int i = LUX; i < RDX; i++) 	Img[RDY * W * 3 + i * 3 + 2] = 255;
	for (int i = LUY; i < RDY; i++) 	Img[i * W * 3 + LUX * 3] = 0;
	for (int i = LUY; i < RDY; i++) 	Img[i * W * 3 + LUX * 3 + 1] = 0;
	for (int i = LUY; i < RDY; i++) 	Img[i * W * 3 + LUX * 3 + 2] = 255;
	for (int i = LUY; i < RDY; i++) 	Img[i * W * 3 + RDX * 3] = 0;
	for (int i = LUY; i < RDY; i++) 	Img[i * W * 3 + RDX * 3 + 1] = 0;
	for (int i = LUY; i < RDY; i++) 	Img[i * W * 3 + RDX * 3 + 2] = 255;
}
...
int main()
{
	...
	*/
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if (95<Cb[i * W + j] && Cb[i * W + j] < 125 && 155<Cr[i * W + j] && Cr[i * W + j] < 185) {
				LUY = i;
				flag = 1;
				break;
			}
	...
	DrawRedRectOutline(Output, W, H, LUX, LUY, RDX, RDY);  //DrawRectOutline 아래에 다음 함수 사용
