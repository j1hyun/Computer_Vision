...
int main()
{
	...
	*/
	...
	for (int a = 160; a<320; a++) {
		for (int i = 0; i < W; i++) {
			wt2 = i / (double)(W - 1);
			Image[a * W * 3 + i * 3] = (BYTE)(255 * wt2);  // Blue
			Image[a * W * 3 + i * 3 + 1] = (BYTE)(255 *(1.0- wt2)); // Green
			Image[a * W * 3 + i * 3 + 2] = (BYTE)(255 * wt2); // Red
		}
	}
	//그라데이션 만들기3(B~Y)
	double wt3;
	for (int a = 0; a<160; a++) {
		for (int i = 0; i < W; i++) {
			wt3 = i / (double)(W - 1);
			Image[a * W * 3 + i * 3] = (BYTE)(255 * (1.0-wt3));  // Blue
			Image[a * W * 3 + i * 3 + 1] = (BYTE)(255 * wt3); // Green
			Image[a * W * 3 + i * 3 + 2] = (BYTE)(255 * wt3); // Red
		}
	}
