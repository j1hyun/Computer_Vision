...
int main()
{
	...
	int Margin = Length / 2; //Margin은 Length에 2를 나눈 몫이다. 이 margin을 기준으로 중심 잡기 위해서 선언

	int WSize = Length * Length; //window size 값 정의

	BYTE* temp = (BYTE*)malloc(sizeof(BYTE) * WSize); // window size 크기 만큼 동적할당

	int W = hInfo.biWidth, H = hInfo.biHeight;  //비트맵 이미지의 넓이와 높이에 관하여 초기화

	int i, j, m, n; //반복문에서 필요한 변수 선언

	for (i = Margin; i < H - Margin; i++) { //y축 의미

		for (j = Margin; j < W - Margin; j++) { //x축 의미

			for (m = -Margin; m <= Margin; m++) {//y축의 값 의미함 i+m을 통해  margin에서 y축으로 얼마큼 떨어져 있는지 나타낼 수 있음

				for (n = -Margin; n <= Margin; n++) {//x축의 값 의미함 j+n을 통해 margin에서 x축으로 얼마큼 떨어져 있는지 나타낼 수 있음

					temp[(m + Margin) * Length + (n + Margin)] = Image[(i + m)*W + j + n]; // 반복문을 통해 각 화소 위치를 정해줌 가운데 값인 temp[margin]으로부터 기본 공식을 통해 각각 아래 위아래에 맞는 값을 넣어 초기화해 준다.

				}

			}

			Output[i * W + j] = Median(temp, WSize); //  window사이즈에 맞춰 median filtering을 진행한 후 output값을 넣어줌
